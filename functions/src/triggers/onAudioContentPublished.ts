  ...
  async (event) => {
    ...
    try {
      ...
      if (finalPreviewPath) {
        ...
        // The preview is already an AAC/M4A file generated by renderAudioDraftPreview.
        // We can use it directly as the master to skip re-rendering.
        logger.info(`[8L1-pipeline] ${contentId}: using approved preview directly as master.`, { finalPreviewPath });
        await bucket.file(finalPreviewPath).download({ destination: tmpMaster });

        // Fast probe to get metadata
        const previewProbe = await probeAudio(tmpMaster);
        masterDurationMs = previewProbe.durationMs;
        masterSizeBytes = fs.statSync(tmpMaster).size;
        masterCodec = previewProbe.codec || 'aac';
        logger.info(`[8L1-pipeline] ${contentId}: preview used as master.`, { durationMs: masterDurationMs });
